...
def peak_pick_then_threshold(activations: np.ndarray) -> np.ndarray:
    ...
    if values.ndim != 2:
        raise ValueError(
            "Timbre-Trap activations must have shape [frames, native_pitches]"
        )
...
def native_to_common_grid(
    ...
    if (
        times.ndim != 1
        or centers.ndim != 1
        or active.shape != (times.size, centers.size)
    ):
    ...
    for index, frequency in enumerate(centers):
        ...
        if (
            21 <= nearest <= 108
            and abs(midi - nearest) * 100.0 <= PITCH_TOLERANCE_CENTS
        ):
    ...
    for common_index, time_seconds in enumerate(common):
        ...
        for midi, native_indices in mapped.items():
            result[common_index, midi - 21] = bool(
                np.any(active[native_index, native_indices])
            )
...
def normalize_frame_output(
    ...
    return TranscriptionOutput(
        None,
        native_to_common_grid(
            native_times, native_pitch_centers_hz, active, common_times
        ),
    )
...
class TimbreTrapAdapter:
    ...
    def load(self, device: str = "cpu") -> None:
        ...
        try:
            ...
            # librosa 1.0.0 lazily imports a Numba-decorated notation module
            # for the official CQT grid conversion. Disable only that JIT
            # path per process; the upstream CQT/model computation is unchanged.
    ...
    def _read_audio(path: Path, sample_rate: int) -> Any:
        ...
        if source_rate != sample_rate:
            ...
            samples = signal.resample_poly(
                samples, sample_rate // gcd, source_rate // gcd
            ).astype(np.float32)
    ...
    def transcribe(self, audio: Path) -> TranscriptionOutput:
        ...
        with torch.inference_mode():
            activations = model.transcribe(
                torch.from_numpy(samples[None, None, :]).to(
        ...
        if values.ndim != 2:
            raise ValueError(
                f"Timbre-Trap returned unexpected activations {values.shape}"
            )
        ...
        return normalize_frame_output(
            native_times, native_hz, values, common_frame_times(frame_count)
        )

    def backward_measurement(self, audio: Path) -> float:
        """Measure the official differentiable forward boundary without updating weights."""
        if self.active_model is None:
            self.load()
        import torch
        ...
        sample_rate = int(self.spec.stock_inference["sample_rate"])
        samples = self._read_audio(Path(audio), sample_rate)
        tensor = torch.from_numpy(samples[None, None, :]).to(
            next(model.parameters()).device
        )
        tensor = model.sliCQ.pad_to_block_length(tensor)
        model.zero_grad(set_to_none=True)
        with torch.enable_grad():
            outputs = model(tensor, consistency=False)
            transcription = outputs[2]
            loss = transcription.square().mean()
            loss.backward()
        model.zero_grad(set_to_none=True)
    ...
    def _frame_count(path: Path) -> int:
        ...
        sample_count = (
            round(count * 22050 / source_rate) if source_rate != 22050 else count
        )
