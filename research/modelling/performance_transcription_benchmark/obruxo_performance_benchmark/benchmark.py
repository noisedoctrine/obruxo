...
def measure_callable(
    call: Callable[[], Any], *, repetitions: int = 10
) -> dict[str, Any]:
...
def _approved_output(path: Path | str) -> Path:
    ...
    if candidate == root or not candidate.is_relative_to(root):
        raise ValueError(
            "benchmark output must be inside the ignored benchmark output area"
        )
    return candidate
...
def _run_candidate_worker(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "obruxo_performance_benchmark.candidate_benchmark_worker",
        ],
        cwd=package_root,
...
def _aggregate_candidate_route(
    rows: Sequence[Mapping[str, Any]], *, route: str, native_batch_sizes: Sequence[int]
) -> dict[str, Any]:
    ...
    for case_index in sorted(
        {
            ...
            for case in row["end_to_end"]["cases"]
        }
    ):
        cases = [
            case
            for row in successful
            for case in row["end_to_end"]["cases"]
            if int(case["case_index"]) == case_index
        ]
        audio_values = {float(case["audio_seconds"]) for case in cases}
        if len(audio_values) != 1:
            raise ValueError(
                "candidate smoke audio duration changed across repetitions"
            )
        wall_summary = aggregate_measurements(
    ...
    total_wall = aggregate_measurements(
        [float(row["end_to_end"]["wall_seconds"]) for row in successful]
    )
    ...
    for field in (
        ...
        if values and any(value != values[0] for value in values[1:]):
            raise ValueError(
                f"candidate resource invariant changed across repetitions: {field}"
            )
        if values:
            memory[field] = values[0]
    for field in ("host_peak_rss_bytes",):
        values = [
            float(item[field]) for item in memories if item.get(field) is not None
        ]
        if values:
    ...
    xpu_values = [
        ...
        if item.get("xpu_memory") is not None
    ]
    if xpu_values:
        if not all(isinstance(value, Mapping) for value in xpu_values):
            raise ValueError(
                "candidate XPU memory measurements have inconsistent shapes"
            )
        memory["xpu_memory"] = {
            key: aggregate_measurements(
                [float(value[key]) for value in xpu_values if key in value]
            )
    ...
    return {
        ...
        "startup": {
            "model_load_seconds": aggregate_measurements(
                [float(row["startup"]["model_load_seconds"]) for row in successful]
            ),
            "first_call_seconds": aggregate_measurements(
                [float(row["startup"]["first_call_seconds"]) for row in successful]
            ),
            "warmup_seconds": aggregate_measurements(
                [float(row["startup"]["warmup_seconds"]) for row in successful]
            ),
        },
        "steady_state": {
            "seconds": aggregate_measurements(
                [float(row["steady_state"]["seconds"]["median"]) for row in successful]
            ),
            "calls_per_second": aggregate_measurements(
                [float(row["steady_state"]["calls_per_second"]) for row in successful]
            ),
        },
        ...
        "end_to_end": {
            ...
            "audio_seconds_per_wall_second": total_audio / total_wall["median"]
            if total_wall["median"]
            else 0.0,
            "cases": e2e_rows,
        },
        "resources": memory,
        "backward": _aggregate_backward(successful),
        "native_batch_sizes": list(native_batch_sizes),
        "quantization": successful[0].get("quantization"),
    }


def _aggregate_backward(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    measured = [
        row.get("backward", {})
        for row in rows
        if (row.get("backward") or {}).get("status") == "measured"
    ]
    if not measured:
        return (
            dict(rows[0].get("backward", {})) if rows else {"status": "not_applicable"}
        )
    values = [float(item["seconds"]) for item in measured]
    first = dict(measured[0])
    first["seconds"] = aggregate_measurements(values)
    first["repetitions"] = len(values)
    return first
...
def _landed_basic_pitch_runtime(
    smoke_manifest: Path, spec: ModelSpec
) -> tuple[dict[str, Any], int]:
    root = _basic_pitch_root()
    report_path = root / "reports" / "backend_benchmark.json"
    if not report_path.is_file():
        raise FileNotFoundError(
            "landed Basic Pitch #24 benchmark report is unavailable"
        )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if (
        not isinstance(report, dict)
        or report.get("model_id") != "spotify-basic-pitch-icassp-2022-v0.4.0"
    ):
    ...
    if (
        not isinstance(smoke_set, dict)
        or smoke_set.get("status") != "ok"
        or int(smoke_set.get("case_count", -1)) != len(cases)
    ):
        raise ValueError(
            "landed Basic Pitch #24 smoke workload does not match the supplied manifest"
        )
    ...
    *,
    ...
    if output.exists() and not force and any(output.iterdir()):
        raise FileExistsError(
            "refusing to overwrite benchmark output without force=True"
        )
    ...
    if spec.model_id == "basic_pitch" and not quantized:
        try:
            ...
            result = {
                ...
                "routes": _unavailable_routes(
                    "dependency_unavailable", quantized=False
                ),
                ...
                "backward": {
                    "status": "not_applicable",
                    "reason": "landed route unavailable",
                },
                "batch_scaling": {},
            }
        else:
            routes = list(report.get("inference", [])) + list(
                report.get("training", [])
            )
            result = {
                "format_version": 1,
    ...
    if not manifest.is_file():
        ...
        try:
            ...
            cases = load_manifest(
                ...
                config,
                allow_derived_render=True,
                allow_missing_derived_audio=False,
            )
            ...
            if any(not case.audio_path.is_file() for case in cases):
                ...
                checkpoint = (
                    getattr(adapter, "checkpoint", None)
                    or root / "artifacts" / "basic_pitch_icassp_2022.pt"
                )
                for route in ("pytorch_cpu", "pytorch_xpu"):
                    ...
                    route_result = _aggregate_candidate_route(
                        repetitions,
                        route=route,
                        native_batch_sizes=spec.native_batch_sizes,
                    )
                        ...
                        route_failures.append(
                            {
                                ...
                                "failure_code": route_result.get("failure_code"),
                            }
                        )
                successful_routes = [
                    route for route in routes if route.get("status") == "ok"
                ]
                if successful_routes:
                    code = None
                    quantization_info = next(
                        (
                            route.get("quantization")
                            for route in successful_routes
                            if route.get("quantization")
                        ),
                        None,
                    )
                else:
                    code = (
                        route_failures[0].get(
            ...
            if source_before is not None:
                try:
                    ...
                    routes = []
                    route_failures = [
                        {"route": "all", "status": "failed", "failure_code": code}
                    ]
                else:
    ...
    status = (
        "ok"
        if any(route.get("status") == "ok" for route in routes)
        else (
            "unavailable"
            if code
            in {
                "dependency_unavailable",
                "smoke_manifest_missing",
                "derived_audio_unavailable",
            }
            else "failed"
        )
    )
    result = {
        ...
        "backward": {
            "status": "not_applicable",
            "reason": "no executable natural differentiable boundary",
        },
        "batch_scaling": {},
