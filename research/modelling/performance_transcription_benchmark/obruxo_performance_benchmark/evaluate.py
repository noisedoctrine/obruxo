...
def _approved_output(path: Path | str) -> Path:
    ...
    if candidate == root or not candidate.is_relative_to(root):
        raise ValueError(
            "evaluation output must be inside the ignored benchmark output area"
        )
    return candidate
...
def _note_events(notes: Sequence[NormalizedNote]) -> list[Any]:
    ...
    return [
        NoteEvent(
            ...
            amplitude=(note.velocity_midi / 127.0)
            if note.velocity_midi is not None
            else 0.0,
            pitch_bend=None,
        )
        for note in notes
...
def score_output(
    reference: Sequence[Any], output: TranscriptionOutput
) -> dict[str, Any]:
    ...
    if output.frame_pitch is not None:
        ...
        result["frames"] = frame_metrics(
            reference, output.frame_pitch.active_midi.astype(np.float64)
        )
    return result


def failure_metrics(
    reference: Sequence[Any], *, frame_model: bool, frame_count: int = 0
) -> dict[str, Any]:
    ...
    if not frame_model:
        ...
        result["frames"] = frame_metrics(
            reference,
            empty_frame_prediction(frame_count).active_midi.astype(np.float64),
        )
    return result


def _coverage(
    eligible: int, successful: int, failed: int, aggregate: Mapping[str, Any]
) -> dict[str, Any]:
...
def _annotate_category_coverage(
    aggregate: dict[str, Any], rows: Sequence[Mapping[str, Any]]
) -> None:
    for field in tuple(aggregate.get("groups", {})):
        values = {
            str(row.get("labels", {}).get(field, "unknown") or "unknown")
            for row in rows
        }
        ...
        for value in sorted(values):
            matching = [
                row
                for row in rows
                if str(row.get("labels", {}).get(field, "unknown") or "unknown")
                == value
            ]
...
def build_quality_views(
    ...
    *,
    ...
    success_only = aggregate_results(
        successful_rows, bootstrap_replicates=bootstrap_replicates, seed=seed
    )
    ...
    for row in materialized:
        ...
        if reference is None:
            raise ValueError(
                "failure penalty requires the corresponding reference notes"
            )
        penalized = dict(row)
        penalized["source_status"] = row.get("status")
        penalized["source_failure_code"] = row.get("failure_code")
        penalized["status"] = "ok"
        penalized["metrics"] = failure_metrics(
            reference,
            frame_model=frame_model,
            frame_count=int(row.get("frame_count", 0)),
        )
        penalized_rows.append(penalized)
    failure_penalized = aggregate_results(
        penalized_rows, bootstrap_replicates=bootstrap_replicates, seed=seed
    )
    ...
    return {
        ...
        "failure_penalized": {
            ...
            "coverage": float(successful / len(materialized)) if materialized else None,
            "aggregate": _coverage(
                len(materialized), successful, failed, failure_penalized
            ),
        },
    }
...
def _frame_count(audio_path: Path) -> int:
    try:
        ...
        return max(
            0,
            int(prepared.original_sample_count * ANNOTATIONS_FPS // AUDIO_SAMPLE_RATE),
        )
...
def _code_identity() -> dict[str, str]:
    ...
    files = {
        ...
        "basic_pitch/constants.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "constants.py",
        "basic_pitch/inference.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "inference.py",
        "basic_pitch/model.py": basic_pitch_root / "obruxo_basic_pitch" / "model.py",
        "basic_pitch/postprocess.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "postprocess.py",
        "basic_pitch/evaluation/aggregate.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "evaluation"
        / "aggregate.py",
        "basic_pitch/evaluation/corpus.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "evaluation"
        / "corpus.py",
        "basic_pitch/evaluation/labels.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "evaluation"
        / "labels.py",
        "basic_pitch/evaluation/metrics.py": basic_pitch_root
        / "obruxo_basic_pitch"
        / "evaluation"
        / "metrics.py",
    }
...
def _adapter_configuration(adapter: object) -> dict[str, Any]:
    configuration = getattr(adapter, "inference_configuration", None)
...
def _pair_resume_identity(
    ...
    identity = {
        ...
        "adapter_configuration": _adapter_configuration(adapter),
        "runtime_identity": dict(runtime_identity),
        "code_identity": dict(code_identity or _code_identity()),
        "backend_contract": {
            "route": f"pytorch_{device}",
            "device": device,
...
def _load_cached_pair(
    ...
    *,
    allow_segment_batch_variant: bool = False,
) -> dict[str, Any] | None:
    try:
    ...
    if value.get("status") != "ok":
        # A prior runtime/OOM/output failure is resumable state, not a
        # successful result. Retry it in a fresh process so transient device
        # failures cannot be silently preserved as the final outcome.
        return None
    stored_identity = value.get("resume_identity_digest", value.get("resume_identity"))
    stored_details = value.get("resume_identity")
    if stored_identity != expected_identity:
        if (
            not allow_segment_batch_variant
            or not isinstance(stored_details, Mapping)
            or expected_details is None
        ):
            return None
        expected_details = dict(expected_details)
        stored_details = dict(stored_details)
        for key in expected_details.keys() | stored_details.keys():
            if key in {"adapter_configuration", "code_identity"}:
                continue
            if stored_details.get(key) != expected_details.get(key):
                return None
        stored_configuration = stored_details.get(
            "adapter_configuration", {"segment_batch_size": 8}
        )
        expected_configuration = expected_details.get("adapter_configuration")
        if not (
            isinstance(stored_configuration, Mapping)
            and isinstance(expected_configuration, Mapping)
            and set(stored_configuration) == {"segment_batch_size"}
            and set(expected_configuration) == {"segment_batch_size"}
            and int(stored_configuration["segment_batch_size"]) > 0
            and int(expected_configuration["segment_batch_size"]) > 0
        ):
            return None
        stored_code = stored_details.get("code_identity", {})
        expected_code = expected_details.get("code_identity", {})
        allowed_code_changes = {
            "adapters/__init__.py",
            "adapters/muscriptor.py",
            "adapters/timbre_trap.py",
            "adapters/yourmt3.py",
            "evaluate.py",
        }
        if not isinstance(stored_code, Mapping) or not isinstance(
            expected_code, Mapping
        ):
            return None
        for key in stored_code.keys() | expected_code.keys():
            if key not in allowed_code_changes and stored_code.get(
                key
            ) != expected_code.get(key):
                return None
    elif expected_details is not None and stored_details != dict(expected_details):
        return None
...
def _row_segment_batch_size(row: Mapping[str, Any]) -> int | None:
    identity = row.get("resume_identity")
    configuration = (
        identity.get("adapter_configuration") if isinstance(identity, Mapping) else None
    )
    if isinstance(configuration, Mapping) and "segment_batch_size" in configuration:
        return int(configuration["segment_batch_size"])
    if isinstance(identity, Mapping) and identity.get("adapter_identity", "").endswith(
        ".YourMT3Adapter"
    ):
        return 8
    return None
...
def _write_unavailable(
    output: Path, run: EvaluationRun, *, reason: str | None = None
) -> None:
    _atomic_json(
        output / "run.json",
        {
    ...
    _atomic_json(
        ...
        {"status": run.status, "failure_code": run.failure_code, "quality": None},
    )


def _landed_quality_views(
    ...
    *,
    ...
    view = {
        ...
        "successful_pairs": successful_pairs,
        "failed_pairs": failed_pairs,
        "coverage": float(successful_pairs / eligible_pairs)
        if eligible_pairs
        else None,
...
def _consume_landed_basic_pitch(
    ...
    if not isinstance(identity, Mapping) or identity.get("manifest_sha256") != _digest(
        manifest
    ):
        raise ArtifactError(
            "landed Basic Pitch baseline does not match the supplied #25 manifest"
        )
    if identity.get("checkpoint_sha256") != spec.checkpoint_sha256:
        raise ArtifactError(
            "landed Basic Pitch baseline checkpoint does not match models.yaml"
        )
    ...
    if pair_count != len(pairs) or successful_pairs + failed_pairs != pair_count:
        raise ArtifactError(
            "landed Basic Pitch baseline pair population does not match the supplied #25 manifest"
        )
...
def evaluate_model(
    ...
    *,
    ...
    if device not in {"cpu", "xpu"}:
        raise ValueError(f"unsupported evaluation device: {device}")
    ...
    if spec.family == "basic_pitch" and quantized:
        ...
        if callable(load):
            load()
        return _consume_landed_basic_pitch(
            spec, adapter, manifest, pairs, output, runtime_identity, code_identity
        )
    ...
    if spec.family != "basic_pitch":
        ...
        try:
            ...
            run = _model_run(
                spec, variant_id, manifest, "failed", failure_code="model_load_failed"
            )
            _write_unavailable(output, run, reason=str(exc))
            return run
    ...
    if quantized:
        ...
        try:
            ...
            if (
                ...
                or result.quantized_linear_modules < 1
            ):
            ...
            failure_code = (
                ...
                if quantization_info is None or quantization_info.get("status") != "ok"
                else "quantized_runtime_failed"
            )
            run = _model_run(
                spec, variant_id, manifest, "failed", failure_code=failure_code
            )
            _write_unavailable(output, run, reason=str(exc))
            return run
    adapter_identity = _adapter_identity(adapter)
    ...
    if not pairs:
        run = _model_run(
            spec, variant_id, manifest, "unavailable", failure_code="no_eligible_pairs"
        )
        _write_unavailable(
            output,
            run,
            reason="the exact landed #25 manifest contains no eligible pairs",
        )
        return run
    rows: list[dict[str, Any]] = []
    references: dict[str, Sequence[Any]] = {}
    observed_segment_batch_sizes: set[int] = set()
    reused_segment_batch_sizes: set[int] = set()
    ...
    for pair in pairs:
        ...
        cached = (
            None
            if force
            else _load_cached_pair(
                pairs_dir / f"{pair.pair_id}.json",
                resume_identity,
                pair.pair_id,
                resume_details,
                allow_segment_batch_variant=spec.family == "yourmt3",
            )
        )
        if cached is not None:
            row = cached
            batch_size = _row_segment_batch_size(row)
            if batch_size is not None:
                observed_segment_batch_sizes.add(batch_size)
                if batch_size != adapter_configuration.get("segment_batch_size"):
                    reused_segment_batch_sizes.add(batch_size)
            rows.append(row)
            continue
        try:
            ...
            row = {
                ...
                "failure_code": None,
                "metrics": metrics,
                "frame_count": output_value.frame_pitch.active_midi.shape[0]
                if output_value.frame_pitch is not None
                else 0,
            }
        except MemoryError:
            row = {
                ...
                "frame_count": _frame_count(pair.audio_path) if frame_model else 0,
            }
            ...
            row = {
                ...
                "failure_code": "invalid_native_output",
    ...
    _atomic_json(
        output / "run.json",
        {
            ...
            "adapter_identity": adapter_identity,
            "adapter_configuration": _adapter_configuration(adapter),
            "segment_batch_sizes_observed": sorted(observed_segment_batch_sizes),
            "cache_reuse_policy": (
                "yourmt3_segment_batch_equivalence"
                if reused_segment_batch_sizes
                else "exact_identity"
            ),
            "runtime_identity": runtime_identity,
            "code_identity": code_identity,
