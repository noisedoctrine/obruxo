...
def _run(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if route not in {"pytorch_cpu", "pytorch_xpu"}:
        return {"status": "runtime_failed", "failure_code": "unsupported_candidate_route", "route": route}
    device_name = "cpu" if route == "pytorch_cpu" else f"xpu:{int(request.get('xpu_index', 0))}"
    if route == "pytorch_xpu" and (not hasattr(torch, "xpu") or not torch.xpu.is_available()):
        return {"status": "unavailable", "failure_code": "torch_xpu_unavailable", "route": route}
    ...
    cases = load_manifest(smoke_root, config, allow_derived_render=True, allow_missing_derived_audio=False)
    if any(not case.audio_path.is_file() for case in cases):
        return {"status": "unavailable", "failure_code": "derived_audio_unavailable", "route": route}
    adapter = adapter_for(spec, Path(str(request["source_root"])), Path(str(request["checkpoint"])))
    ...
    if bool(request.get("quantized", False)):
        if route != "pytorch_cpu":
            return {"status": "unavailable", "failure_code": "quantized_cpu_only", "route": route}
        ...
        if quantization.status != "ok" or quantization.model is None:
            return {"status": "failed", "failure_code": "quantization_unsupported", "route": route, "quantization": quantization_info}
        adapter.bind_model(quantization.model)
        if getattr(adapter, "active_model", None) is not quantization.model:
            return {"status": "failed", "failure_code": "quantized_runtime_failed", "route": route, "quantization": quantization_info}
    ...
    resources: dict[str, Any] = {
        ...
        "checkpoint_bytes": file_size(Path(str(request["checkpoint"])).resolve(strict=True)),
    ...
    for case in cases:
        ...
        e2e_rows.append({"case_index": case.case_index, "audio_seconds": audio_seconds, "wall_seconds": float(wall), "status": "ok", "output_has_notes": output.notes is not None})
    ...
    batch_scaling = {
        str(size): {"status": "not_applicable", "reason": "adapter exposes only full-clip stock transcription; synthetic serial calls are not native batching"}
        for size in (1, 2, 4, 8)
    }
    return {
        "status": "ok",
        "failure_code": None,
        "route": route,
        "runtime": {"python": sys.version.split()[0], "numpy": np.__version__, "torch": torch.__version__, "psutil": psutil.__version__},
        "startup": {"model_load_seconds": float(model_load_seconds), "first_call_seconds": float(first_seconds), "warmup_seconds": float(warmup_seconds)},
        "steady_state": {"median_seconds": _stats(timed)["median"], "seconds": _stats(timed), "calls_per_second": float(1.0 / _stats(timed)["median"])},
        "batch_scaling": batch_scaling,
        "end_to_end": {"status": "measured", "cases": e2e_rows, "wall_seconds": float(sum(row["wall_seconds"] for row in e2e_rows))},
        "resources": resources,
        "backward": {"status": "not_applicable", "reason": "adapter does not expose a natural differentiable boundary"},
...
def main() -> int:
    try:
        ...
        result = {"status": "runtime_failed", "failure_code": "candidate_runtime_error", "error_type": type(exc).__name__}
