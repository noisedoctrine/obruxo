...
def _labels() -> dict[str, str]:
    return {
        "polyphony_class": "monophonic",
        "duration_class": "short",
        "note_density_class": "low",
        "pitch_register_class": "mid",
        "instrument": "unknown",
        "genre": "unknown",
        "type": "unknown",
        "vital_style": "unknown",
...
def test_failure_penalty_routes_failed_note_through_landed_metrics() -> None:
    ...
    rows = [
        {
            "pair_id": "synthetic-good",
            "preset_id": None,
            "labels": _labels(),
            "status": "ok",
            "metrics": note_metrics(reference, [good]),
        },
        {
            "pair_id": "synthetic-failed",
            "preset_id": None,
            "labels": _labels(),
    ...
    views = build_quality_views(
        rows,
        {"synthetic-good": reference, "synthetic-failed": reference},
        frame_model=False,
    ...
    assert (
        views["failure_penalized"]["aggregate"]["failure_analysis"]["timing"]["onset"][
            "support"
        ]
        == 0
    )
