...
def _counts(
    reference_count: int, prediction_count: int, true_positive: int
) -> dict[str, Any]:
    ...
    return {
        ...
        "false_positive_fraction": _safe_ratio(false_positive, prediction_count)
        if prediction_count
        else None,
    }
...
def _note_arrays(
    notes: Sequence[ReferenceNote | NoteEvent],
) -> tuple[np.ndarray, np.ndarray]:
    intervals = np.asarray(
        [
            (note.start_time_s, note.end_time_s)
            if isinstance(note, NoteEvent)
            else (note.onset_s, note.offset_s)
            for note in notes
        ],
        dtype=np.float64,
    ).reshape(-1, 2)
    pitches = np.asarray(
        [_midi_to_hz(note.pitch_midi) for note in notes], dtype=np.float64
    )
...
def _velocity_summary(
    ...
    errors = [
        round(127 * predicted[est].amplitude) - reference[ref].velocity
        for ref, est in matches
    ]
    ...
    if len(errors) >= 2:
        reference_values = np.asarray(
            [reference[ref].velocity for ref, _ in matches], dtype=np.float64
        )
        predicted_values = np.asarray(
            [round(127 * predicted[est].amplitude) for _, est in matches],
            dtype=np.float64,
        )
        if np.ptp(reference_values) != 0 and np.ptp(predicted_values) != 0:
            summary["pearson"] = float(
                np.corrcoef(reference_values, predicted_values)[0, 1]
            )
...
def _pitch_confusion(
    ...
    for ref_index, ref in enumerate(reference):
        ...
        candidates = [
            ...
            if est_index not in matched_prediction
            and abs(est.start_time_s - ref.onset_s) <= ONSET_TOLERANCE_SECONDS
        ]
        if candidates:
    ...
    deltas = [
        predicted[est].pitch_midi - reference[ref].pitch_midi for ref, est in assigned
    ]
    octave_errors = sum(delta != 0 and delta % 12 == 0 for delta in deltas)
    ambiguous_reference_count = sum(
        1
        for ref_index, candidates in candidate_by_reference.items()
        if len(candidates) != 1 or len(candidate_by_prediction[candidates[0]]) != 1
    )
    assigned_reference = {ref for ref, _ in assigned}
    assigned_prediction = {est for _, est in assigned}
    unassigned_reference_count = (
        len(reference) - len(matched_reference) - len(assigned_reference)
    )
    unassigned_prediction_count = (
        len(predicted) - len(matched_prediction) - len(assigned_prediction)
    )
    return {
        ...
        "unassigned_error_count": unassigned_reference_count
        + unassigned_prediction_count,
...
def note_metrics(
    reference: Sequence[ReferenceNote], predicted: Sequence[NoteEvent]
) -> dict[str, Any]:
    """Compute the fixed event-level metrics without loading models or writing files."""
    onset_pitch_matches = match_note_indices(
        reference, predicted, include_offsets=False
    )
    offset_matches = match_note_indices(reference, predicted, include_offsets=True)
    timing = {
        "onset": _error_summary(
            [
                predicted[est].start_time_s - reference[ref].onset_s
                for ref, est in onset_pitch_matches
            ]
        ),
        "offset": _error_summary(
            [
                predicted[est].end_time_s - reference[ref].offset_s
                for ref, est in onset_pitch_matches
            ]
        ),
    ...
    return {
        "onset_pitch": _counts(
            len(reference), len(predicted), len(onset_pitch_matches)
        ),
        "onset_pitch_offset": _counts(
            len(reference), len(predicted), len(offset_matches)
        ),
        "out_of_range_reference_notes": sum(
            not MIDI_LOW <= note.pitch_midi <= MIDI_HIGH for note in reference
        ),
...
def frame_target(reference: Sequence[ReferenceNote], n_frames: int) -> np.ndarray:
    ...
    for note in reference:
        if not MIDI_LOW <= note.pitch_midi <= MIDI_HIGH:
            continue
        target[:, note.pitch_midi - MIDI_LOW] |= (times >= note.onset_s) & (
            times < note.offset_s
        )
    return target
...
def frame_metrics(
    reference: Sequence[ReferenceNote], note_posterior: np.ndarray
) -> dict[str, Any]:
    ...
    result = _counts(
        int(np.count_nonzero(target)), int(np.count_nonzero(prediction)), true_positive
    )
    ...
    result.update(
        {
            ...
            "reference_onset_posterior": _distribution(
                np.asarray(onset_values, dtype=np.float64)
            ),
        }
    )
    return result
...
def evaluate_notes_and_frames(
    ...
    return {
        ...
        "frames": frame_metrics(reference, note_posterior),
    }
