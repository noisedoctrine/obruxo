...
def test_openvino_cpu_dynamic_batch_records_canonical_parity_gate() -> None:
    ...
    converted = ov.convert_model(
        model, example_input=torch.zeros((1, 43_844, 1), dtype=torch.float32)
    )
    ...
    for batch_size in (1, 2, 4, 8):
        ...
        reference = {
            name: value.detach().numpy() for name, value in reference_values.items()
        }
        ...
        candidate = dict(
            zip(("note", "onset", "contour"), candidate_values, strict=True)
        )
        ...
        assert summary["parity_passed"] is (
            summary["pitch_bend_element_disagreements"] == 0
        )
