...
class StrategyGridPartialReportTests(unittest.TestCase):
    ...
    def test_complete_13a_report_keeps_final_analysis_gated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("exploratory sweep axis", text)
            self.assertIn("eligibility epsilons `1e-2`, `1e-3`, and `1e-4` for 135 rows", text)
            ...
            self.assertIn("sweep planned", html)
            self.assertIn("45 LayerClip0To1 strategies independently", html)
