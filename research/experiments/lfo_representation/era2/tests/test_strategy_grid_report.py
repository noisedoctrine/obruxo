...
class StrategyGridPartialReportTests(unittest.TestCase):
    def test_partial_report_uses_shards_and_preserves_source(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('id="global-family"', html)
            self.assertIn('id="global-policy"', html)
            self.assertIn('id="global-schedule"', html)
            self.assertIn('id="global-budget"', html)
            self.assertIn('id="global-normalization"', html)
