...
$OutDir = Join-Path $Root "artifacts\additive_finalization_8_screen"
$Stdout = Join-Path $OutDir "experiment8_background_stdout.log"
$Stderr = Join-Path $OutDir "experiment8_background_stderr.log"
$Done = Join-Path $OutDir "COMPLETED_EXPERIMENT_8.txt"
$env:MPLCONFIGDIR = Join-Path $Root "artifacts\mpl"

$Host.UI.RawUI.WindowTitle = "Experiment 8 Monitor"

while ($true) {
    Clear-Host
    Write-Host "Experiment 8 Monitor" -ForegroundColor Cyan
    ...
    if (Test-Path $Done) {
        Write-Host ""
        Write-Host "Experiment 8 completed:" -ForegroundColor Green
        Get-Content $Done
        Write-Host ""
